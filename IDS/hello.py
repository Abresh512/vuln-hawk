...
BASELINE_INTERVAL = 60  # measure baseline every minute
THRESHOLD_MULTIPLIER = 5  # how many times above baseline before alert
...
def detect_syn(packet):
    if packet.haslayer(TCP) and packet.haslayer(IP):
        ...
        if tcp.flags & 0x02:  # SYN flag
            ...
            # remove old SYNs outside the window
            ...
            baseline = baseline_rate[src_ip] or 5  # default baseline if none yet
            if syn_rate > baseline * THRESHOLD_MULTIPLIER:
                print(f"[ALERT] Possible SYN flood from {src_ip} — rate: {syn_rate:.2f}/s (baseline: {baseline:.2f}/s)")
