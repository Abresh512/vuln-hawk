...
# ---------- Config ----------
window_sec = 10         # window for SYN detection
threshhold = 60    
treshhold_multiplier = 5           # SYN threshhold to trigger alert
alert_suppress_secs = 30 # suppress repeated alert for same IP for this many seconds
active_ip = 30          # seconds: an IP is "active" if seen within this many seconds


alerts_log = "alerts.log"    #Log IPs that have syn flood bahaviour
syn_keyword = ("syn", "syn flood", "syn scan", "tcp:flags=syn", "syn-ack")
# ----------------------------
...
def log_alert_line(ts_iso, ip, message):
    ...
    # Append alert to alerts.log single-line CSV alert: timestamp,ip,message
    ...
    try:
        ...
        if packet.haslayer(TCP):
            ...
            if tcp_layer.flags & 0x02:
                with _lock:
                    syn_pkt = syn_packets[src]
                    syn_pkt.append(now)
                    # remove old timestamps outside sliding window
                    while syn_pkt and (now - syn_pkt[0]) > window_sec:
                        syn_pkt.popleft()

                    # if count above threshhold and not recently alerted, log it
                    if len(syn_pkt) > threshhold * treshhold_multiplier:
                        ...
                        if now - last > alert_suppress_secs:
                            ...
                            msg = f"SYN flood suspected to {dst} ({len(syn_pkt)} SYNs in {window_sec}s)"
...
def parse_alerts_file(path=alerts_log):
    ...
    with open(path, "r") as f:
        for line in f:
            ...
            is_syn = any(k in lower_msg for k in syn_keyword)
            alerts.append({
                "timestamp": timestamp,
                "ip": ip,
                "message": message,
...
def get_active_ips(window=active_ip, top_n=50):
    """
