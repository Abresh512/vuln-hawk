async function fetchAlerts() {
  try {
    ...
    if (!alerts.length) {
      const noAlerts = document.createElement("tr");
      noAlerts.innerHTML = `<td colspan="4" class="no-alerts">No alerts detected</td>`;
      tbody.appendChild(noAlerts);
      return;
    }
    ...
    for (const a of alerts) {
      ...
      tr.innerHTML = `
        ...
        <td>${a.ip || "-"}</td>
        <td>${a.message || "-"}</td>
        <td>${a.is_syn ? '<span class="tag syn">SYN</span>' : "—"}</td>
      `;
